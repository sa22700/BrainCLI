...
DEFAULT_LANG     = os.environ.get("BRAINCLI_SEARCH_LANG", "en")
...
def _pretty_lines(items: List[dict], limit: int) -> str:
    ...
    maxn = max(1, limit)
    for r in items[:maxn]:
        title   = _clean(r.get("title") or "") or "(no title)"
...
def _build_ssl_context() -> Optional[ssl.SSLContext]:
    ...
    if CA_FILE:
        try:
            ...
            log_error(f"Failed to load CA file: {CA_FILE} ({e})")
            raise RuntimeError(f"Failed to load CA file: {CA_FILE} ({e})")
    return ctx
...
def fetch_url(args: str = "") -> str:
    try:
        ...
        if not bases:
            return "Search server URL is missing."
        ...
        if last_err:
            ...
            if isinstance(e, HTTPError):
                raise e
            raise RuntimeError(f"Last tried endpoint failed: {base} ({e})")
        raise RuntimeError("Search failed for an unknown reason.")
        ...
        try:
            ...
            body = ""
        msg = f"Search failed: HTTP {e.code} {e.reason}"
        if body:
            msg += f"\nServer response: {body[:500]}"
        log_error(msg)
        return msg
